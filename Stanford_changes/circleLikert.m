...
%% Lines added by Anna Lange

scanner = 0  % 0 for testing, change this to 1 for scanning
...
while true
    if scanner==1
        ...
        key = GetKey({'right' 'left' 'g'},[],[],-3);
        if sum(strcmp(key, 'right'))>0 && c.slidepos < c.numEls
            c.slidepos = c.slidepos + 1;
            circleRater(c,bottomtext,scaletext,colors,c.Window,scanner);
            
        elseif sum(strcmp(key, 'left'))>0 && c.slidepos > 1
            c.slidepos = c.slidepos - 1;
            circleRater(c,bottomtext,scaletext,colors,c.Window,scanner);
